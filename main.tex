...
\begin{theorem}[\cite[Theorem 2.2.8]{riehl2014categorical}]
    设 $\mathcal{C}$ 和 $\mathcal{D}$ 是模型范畴, $Q$ 是 $\mathcal{C}$ 的余纤维性替换函子, $F\colon \mathcal{C} \rightarrow \mathcal{D}$ 保持余纤维性对象之间的弱等价. 
    那么 $FQ$ 是 $F$ 的左导出函子.

    对偶地, 设 $R$ 是 $\mathcal{C}$ 的纤维性替换函子, $F|\mathcal{C}_\mathrm{f}$ 保持弱等价, 那么 $RF$ 是 $F$ 的右导出函子.
\end{theorem}
\begin{proof}
