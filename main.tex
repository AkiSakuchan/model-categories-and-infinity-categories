...
\begin{theorem}
    设 $\mathcal{C}$ 和 $\mathcal{D}$ 是模型范畴, $Q,q$ 是 $\mathcal{C}$ 的余纤维性替换函子和相应的自然变换, $F\colon \mathcal{C} \rightarrow \mathcal{D}$ 保持余纤维性对象之间的弱等价. 
    那么 $FQ$ 和 $Fq \colon FQ \rightarrow F$ 构成 $F$ 的左导出函子 $\mathbb{L}F$.

    对偶地, 设 $R,r$ 是 $\mathcal{C}$ 的纤维性替换函子和相应的自然变换, $F|\mathcal{C}_\mathrm{f}$ 保持弱等价, 
...
\begin{proposition}
    设 $\mathcal{C}$ 和 $\mathcal{D}$ 是模型范畴, $Q$ 是 $\mathcal{C}$ 的余纤维性替换函子, $F\colon \mathcal{C} \rightarrow \mathcal{D}$ 保持余纤维性对象之间的弱等价.
...
\begin{definition}
    设 $\mathcal{C},\mathcal{D}$ 是模型范畴, $F \colon \mathcal{C} \leftrightarrows \mathcal{D} \colon G$ 是一对伴随函子.
    如果 $F$ 保持余纤维性对象之间的弱等价, 那么 $F$ 称为{\bf 左 Quillen 函子}; 如果 $G$ 保持纤维性对象之间的弱等价, 那么 $G$ 称为{\bf 右 Quillen 函子};
    如果这两者都成立, 则 $(F,G)$ 称为{\bf Quillen 伴随}.
\end{definition}
根据 Ken Brown 引理 \ref{KenBrown-lemma}, 如果 $F$ 保持余纤维化和平凡余纤维化, 那么 $F$ 就是左 Quillen 函子; 对偶地 $G$ 保持纤维化和平凡纤维化, 那么 $G$ 就是右 Quillen 函子.
